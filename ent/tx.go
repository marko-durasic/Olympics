...
func (tx *Tx) init() {
	tx.CombinedSport = NewCombinedSportClient(tx.config)
	tx.IndividualSport = NewIndividualSportClient(tx.config)
	tx.TeamSport = NewTeamSportClient(tx.config)
}
...
// applies a query, for example: CombinedSport.QueryXXX(), the query will be executed
